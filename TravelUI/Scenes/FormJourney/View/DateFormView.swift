...
class DateFormView: UIView {
    ...
    class func instanceFromNib() -> DateFormView {
        return UINib(nibName: identifier, bundle: TravelUI.shared.bundle).instantiate(withOwner: nil, options: nil)[0] as! DateFormView
    }
    ...
    private func initIconImageView() {
        iconImageView.image = UIImage(named: "calendar", in: TravelUI.shared.bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
        iconImageView.tintColor = Configuration.Color.main
    }
    
    private func initArrowImageView() {
        arrowIconImageVIew.image = UIImage(named: "arrow_down", in: TravelUI.shared.bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
