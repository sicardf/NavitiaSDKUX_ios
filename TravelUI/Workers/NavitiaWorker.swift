...
class NavitiaWorker: NavitiaWorkerProtocol {
    ...
    func fetchJourneys(journeysRequest: JourneysRequest, completionHandler: @escaping NavitiaFetchJourneysCompletionHandler) {
        if TravelUI.shared.navitiaSDK != nil {
            let journeyRequestBuilder = TravelUI.shared.navitiaSDK.journeysApi.newCoverageRegionJourneysRequestBuilder()
    ...
    func fetchBss(coord: (lat: Double, lon: Double), distance: Int32, id: String, completionHandler: @escaping (Poi?) -> Void) {
        if TravelUI.shared.navitiaSDK != nil {
            let poisRequestBuilder = TravelUI.shared.navitiaSDK.poisApi.newCoverageLonLatUriPoisRequestBuilder()
                .withLat(coord.lat)
                .withLon(coord.lon)
    ...
    func fetchPlaces(coverage: String, q: String, coord: (lat: String?, lon: String?), completionHandler: @escaping (Places?) -> Void) {
        if TravelUI.shared.navitiaSDK != nil {
            let placesRequestBuilder = TravelUI.shared.navitiaSDK.placesApi.newCoverageRegionPlacesRequestBuilder()
                .withRegion(coverage)
                .withQ(q)
    ...
    func fetchPhysicalMode(coverage: String, completionHandler: @escaping ([PhysicalMode]?) -> Void) {
        if TravelUI.shared.navitiaSDK != nil {
            let physicalModesRequestBuilder = TravelUI.shared.navitiaSDK.physicalModesApi.newCoverageRegionPhysicalModesRequestBuilder()
                .withRegion(coverage)
    ...
    func fetchCoord(lon: Double?, lat: Double?, completionHandler: @escaping (DictAddresses?) -> Void) {
        if TravelUI.shared.navitiaSDK != nil {
            let coordRequestBuilder = TravelUI.shared.navitiaSDK.coordApi.newCoordLonLatRequestBuilder()
                .withLat(lat)
                .withLon(lon)
